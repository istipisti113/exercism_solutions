...
pub fn convert(number: &[u32], from_base: u32, to_base: u32) -> Result<Vec<u32>, Error> {
    ...
    for pos in (0..number.len()).rev() {
        ...
        num_in_base_10 += from_base.pow((number.len()-1 - pos ) as u32) * number[pos]
    }
    println!("{num_in_base_10}");
    if num_in_base_10 == 0 {return Ok(vec![0]);}
